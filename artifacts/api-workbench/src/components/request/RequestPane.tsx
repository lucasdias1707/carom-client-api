...
export function RequestPane({ request, sending, onSend, onCancel }: RequestPaneProps) {
  const { state, dispatch, variables, variableTable, chainFor, unsavedIn, t, tNodes, dataLanguage } = useWorkspace();
  ...
  const copyAsCurl = async () => {
    try {
      await navigator.clipboard.writeText(toCurl(prepareRequest(request, variables, { folders: chain, language: dataLanguage })));
      ...
      >
        <div className="pane-tabbar">
          ...
          <IconButton
            ...
            onClick={() => {
              navigator.clipboard?.writeText(prepareRequest(request, variables, { folders: chain, language: dataLanguage }).url);
