 ...
 *
 * The reroll button sits in the dialog footer rather than in here, so it stays
 * put while this list scrolls. That is why `seed` is a prop: the control and
 * the thing it controls ended up on opposite sides of the scroll container.
 */
export function GeneratedVariables({ seed }: { seed: number }) {
  const { t, tNodes, dataLanguage } = useWorkspace();
  const { toast } = useToast();
  const [filter, setFilter] = useState('');

  /*
    The samples are worked out once per seed rather than on every render: a
    list that reshuffled itself whenever React repainted would be unreadable,
    and the point of rerolling is to see the shape of the data, not to watch
    it move.
  */
...
export function Reroll({ onReroll }: { onReroll: () => void }) {
  const { t } = useWorkspace();
  return (
    <Button variant="ghost" onClick={onReroll} data-testid="button-reroll-samples">
